<h1 align="center">Привет 👋, я Бахтовар</h1>
<h3 align="center">💻 Web & Backend Developer</h3>

---

## 🚀 Обо мне

- 🎓 Студент и разработчик  
- 🔥 Изучаю Web и Backend разработку  
- 🧠 Работаю с **HTML, CSS, JS, PHP, C#**  
- 🚀 Создаю админ-панель **YAMIX**  
- 📈 Хочу объединить бизнес и IT  

---

## 🛠 Технологии и инструменты

<p align="center">
  <img src="https://skillicons.dev/icons?i=html,css,js,bootstrap,php,cs,dotnet,mysql,git,github,vscode" />
</p>

---

## 📊 GitHub статистика

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=IsmatjonovBB&show_icons=true&theme=tokyonight" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=IsmatjonovBB&layout=compact&theme=tokyonight" />
</p>

---

## ⚡ Активность

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=IsmatjonovBB&theme=tokyo-night" />
</p>

---

## 🌍 Связь

- 💬 Всегда открыт к идеям  
- ⭐ Люблю автоматизацию и стартапы  

---

<p align="center">
  🚀 <strong>Let's build something great!</strong>
</p>
